...
function saveRecordImage(data) {
  ...
  // --- แก้ไขจุดนี้: เพิ่มเงื่อนไข data.imageType === 'id' ---
  else if (data.imageType === 'idcard' || data.imageType === 'id') { colIndex = 10; prefix = "ID"; }
  
  if (colIndex === 0) return { success: false, message: "ประเภทรูปไม่ถูกต้อง (" + data.imageType + ")" };
...
function calculateParkingFee(start) {
  ...
  for (let i = 1; i <= totalHours; i++) {
    ...
    if (((i - 1) % 24) + 1 === 24) { 
        totalPrice += currentCyclePrice; currentCyclePrice = 0;
    }
  }
